...
export default function Home() {
  const { state, setState, isSyncing } = useRevisionSync();
  const [isPending, startTransition] = useTransition();
  const [session, setSession] = useState({ orgId: '', userId: '' });

  // Handle Initial Data Fetch and Session Init
  useEffect(() => {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        if (result.success && result.newState) {
          // Immediately sync local state to prevent "flicker" back to old state
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden bg-zinc-950 font-mono text-zinc-100">
      <header className="flex items-center justify-between border-b border-zinc-800 px-4 py-2 bg-zinc-950/80 backdrop-blur-md z-10 shrink-0">
        ...
        <div className="flex items-center gap-4 text-[8px] uppercase tracking-tighter text-zinc-500">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-6 custom-scrollbar">
          <div className="max-w-3xl mx-auto flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
              ...
              <div className="flex flex-col gap-3">
                <div className="text-[9px] text-zinc-500 uppercase tracking-widest">System_Documentation</div>
                <div className="border border-zinc-800 p-4 bg-zinc-900/20 text-[10px] text-zinc-400 leading-relaxed space-y-2">
                  <p>
                    <span className="text-zinc-100 font-bold">ISOLATION:</span> Locked to <span className="text-zinc-200">{session.orgId}</span>.
                  </p>
                  <p>
                    <span className="text-zinc-100 font-bold">ENGINE:</span> Pusher WebSockets.
                  </p>
